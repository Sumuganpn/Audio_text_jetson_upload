...
if (isset($_POST['btn-download'])) {
	$id = $_POST['music'];
	$sql = "SELECT * FROM upload_songs WHERE songID='$id'"; //Get music details from database
	$result = $conn->query($sql);
	if ($result->num_rows > 0) {
		$row = $result->fetch_assoc();

		//Increase downloads (Counter)
		$sql = "UPDATE upload_songs SET songDownloads=songDownloads+1 WHERE songID='$id'";
...
if (isset($_POST['btn-like'])) {
	$id = $_POST['music'];
	$sql = "UPDATE upload_songs SET songLike=songLike+1 WHERE songID='$id'";
...
if (isset($_POST['btn-dislike'])) {
	$id = $_POST['music'];
	$sql = "UPDATE upload_songs SET songDislikes=songDislikes+1 WHERE songID='$id'";
	if ($conn->query($sql) === TRUE) {
			?>
			<script>
				window.location.href='download.php?music=<?php echo $id;?>';
...
if (isset($_GET['music'])){
	$id = $_GET['music'];
	$sql = "SELECT * FROM upload_songs WHERE songID='$id'";
	$result = $conn->query($sql);
	include"function.time-ago.php";
	if ($result->num_rows > 0) {
		$row = $result->fetch_assoc();

		$view = "UPDATE upload_songs SET songViews=songViews+1 WHERE songID='$id'";
		$conn->query($view);
	}else{
